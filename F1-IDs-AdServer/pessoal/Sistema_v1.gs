...
function atualizarUsuarios() {
  ...
  // Carrega admins uma vez — evita leitura repetida por usuário
  ...
  dados.forEach((u, i) => {
    ...
    if (perfil === CONFIG.perfis.operador || perfil === CONFIG.perfis.dev) {
      ...
      // Formata só se cabeçalho estiver incorreto
...
function _protegerAbaComAdmins(sheet, emailUsuario, perfil, acessoExtra, admins, dados, col) {
  try {
    ...
    // Filtra apenas e-mails válidos antes de aplicar
...
// Mantida para compatibilidade com Seguranca.gs
