// ============================================================
// NC Tool | F3 — Validator  ·  Code.gs  (Backup — AfSo)
// ============================================================
...
function doGet(e) {
  ...
  var result;
  try {
    if      (action === 'ping')           result = { ok: true, env: 'pessoal' };
...
function salvarLog(payload) {
  try {
    ...
    if (!log) {
      ...
      log.appendRow(['ID','Timestamp','Usuário','Cliente','Fonte','Arquivo','Plataforma','Total','Corretos','Erros','Score%']);
      log.getRange(1,1,1,11).setFontWeight('bold').setBackground('#0F1B6E').setFontColor('#ffffff');
    }
    if (!det) {
      det = ss.insertSheet('Erros');
      det.appendRow(['LogID','Timestamp','Cliente','Plataforma','Nível','String','Regra','Campo','Valor','Mensagem','Sugestão']);
